...
class BlueBlob(Blob):
    ...
    def __add__(self, other_blob):
        ...
        if other_blob.color == (255, 0, 0):
            ...
            other_blob.size += currentSize
            other_blob.change_movement_range(int(currentSize / 10))

            #sentdex was doing this.. not sure why
            # self.size -= other_blob.size
...
#this could be simplified by this code:
#math.sqrt((b2.x - b1.x)**2 + (b2.y - b1.y)**2) < (b1.size + b2.size)
def is_touching(b1, b2):
    #finding the norm... distance between 2 objects on a plane
    #euclidian distance and norm in machine learning series
...
def main():
    #create dictionaries of blobs.  a dictionary contains a key and a value.  in this case it's i and the blob.
